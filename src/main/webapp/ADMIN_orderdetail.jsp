...
<head>
	...
	<script>
	function confirmBill(param) {
	  window.location.href = 'CONTROLLER_ADMIN?modget=confirm&madonhang=' + param;
	}
	function canBill(param) {
	  window.location.href = 'CONTROLLER_ADMIN?modget=can&madonhang=' + param;
	}
...
<body data-bs-spy="scroll" data-bs-target="#header" tabindex="0">
	<%
		...
		DONHANG order = (DONHANG) request.getAttribute(nameorder);
		...
		float subtotal2 = (float) request.getAttribute("subtotal2");
    %>
			       ...
			       <td style="padding-bottom:0px;" >
			       		...
			       		<div class=" d-flex justify-content-between align-items-center" >
		                     ...
		                     <% if(amountTotal_confirm == amountTotal_confirm2) { %>
	                      	...
	                      	<% } else { %>
	                      	 	<div class="ms-5 pb-0 me-4"><strong class="mx-2">Order Total:</strong> <s><%= amountTotal_confirm %></s> <%= amountTotal_confirm2 %></div>
	                      	<% } %>
		                 </div>
			       </td>
...
<% if (session.getAttribute("cnw") != null) { %>
  <script>
            ...
            <div class="p-3 p-lg-5 border" style=" position: sticky; top: 70px;">
             ...
             <div class="form-group row">
                <div class="col-md-6">
                  ...
                  <input type="text" class="form-control" id="maKH" name="maKH"  value="<%= order.getMaKhachHang() %>" readonly>
                </div>
                <div class="col-md-6">
                  <label for="maDH" class="text-black">Bill Code </label>
                  <input type="text" class="form-control" id="maDH" name="maDH" value="<%= order.getMaDonHang() %>" readonly>
                </div>
              </div>
              <div class="form-group row">
                <div class="col-md-12">
                  <label for="ngayDat" class="text-black">Date Order </label>
                  <input type="text" class="form-control" id="ngayDat" name="ngayDat" value="<%= order.getNgayDatHang() %>" readonly>
                </div>
              </div>
            
              <div class="form-group">
                <label for="hoTen" class="text-black">Full Name </label>
                 <input type="text" class="form-control" id="hoTen" name="hoTen" value="<%= khhang.getHoTen() %>" readonly>
               
              </div>
               <div class="form-group row ">
                <div class="col-md-6">
                  <label for="email" class="text-black">Email Address </label>
                  <input type="text" class="form-control" id="email" name="email" value="<%= khhang.getEmail() %>" readonly>
                </div>
                <div class="col-md-6">
                  <label for="soDT" class="text-black">Phone Number </label>
                  <input type="text" class="form-control" id="soDT" name="soDT" value="<%= khhang.getSDT() %>"  readonly>
                </div>
              </div>
              <div class="form-group row">
                <div class="col-md-12">
                  <label for="address" class="text-black">Address </label>
                  <input type="text" class="form-control" id="address" name="address" value="<%= khhang.getDiaChi() %>" readonly>
                </div>
              </div>
              ...
              <div class="col-md-12">
              	 <div class=" d-flex justify-content-between " >
              	 	<h2 class="h3 mb-3 text-black mt-2 fw-bolder " >Order Product</h2>
	                    ...
	                    <div class="ms-5 mt-2 pb-0 text-danger">*
						    <%
						        int tinhTrang = order.getTinhTrang();
						        if (tinhTrang == 0) {
						    %>
						            To confirm
						    <%
                   ...
                   <% for (int j = 0; j < orderdetail.size(); j++) { %>
                      <tr>
                       ...
                       <td>
                       		...
                       		<% if(orderdetail.get(j).getGiaBan() == orderdetail.get(j).getGiaUuDai()) { %>
                       			<h4><%= orderdetail.get(j).getGiaBan()%></h4>
                       		<% } else { %>
	                       		<h4><s><%= orderdetail.get(j).getGiaBan()%></s></h4>
	                       		<h4><%= orderdetail.get(j).getGiaUuDai()%></h4>
                       		<% } %>
                       </td>
                      ...
                      <td></td>
                        ...
                        <% if(subtotal == subtotal2) { %>
                       		...
                       		<% } else { %>
	                       		<td class="text-black"><s><%= subtotal %></s> <%= subtotal2 %></td>
                       		<% } %>
                      </tr>
                      <tr>
                      <td></td>
                        <td class="text-black font-weight-bold "><strong>Shipping Fee</strong></td>
                        <td class="text-black"><%= fee %></td>
                      </tr>
                      <tr>
                      	<td></td>
                        <td class="text-black font-weight-bold"><strong>Order Total</strong></td>
                        <% if(subtotal == subtotal2) { %>
                       			<td class="text-black font-weight-bold"><strong><%= subtotal + fee %></strong></td>
                       		<% } else { %>
	                       		<td class="text-black font-weight-bold"><strong><%= subtotal2 + fee %></strong></td>
                       		<% } %>
                      </tr>
                    </tbody>
                  </table>
					
					<% if(session.getAttribute("selectOrder").equals("confirm")) { %>
       
                  <div class="row mb-5">
                    <div class="col-md-6 ">
                      <button class="btn btn-primary d-block mx-auto mb-0" style="border-radius: 20px;height: 50px;width: 150px; " onclick="confirmBill('<%= order.getMaDonHang()%>')">Confirm Bill</button>
                    </div>
                    <div class="col-md-6">
                      <button class="btn btn-primary btn-sm btn-block"style="border-radius: 20px;height: 50px;width: 150px; margin-left: 50px;" onclick="canBill('<%= order.getMaDonHang()%>')">Cancel Bill</button>
                    </div>
                  </div>
            		<% } %>
